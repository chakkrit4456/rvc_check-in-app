...
SELECT 
    ...
    CASE 
        WHEN COUNT(p.id) > 0 THEN (COUNT(ar.id)::numeric / COUNT(p.id)::numeric) * 100
        ELSE 0
    END as attendance_rate
FROM departments d
...
CREATE OR REPLACE VIEW classroom_attendance_stats AS
SELECT 
...
LEFT JOIN profiles p ON c.id = p.classroom_id AND p.role = 'student'
LEFT JOIN attendance_records ar ON p.id = ar.student_id
...
GRANT SELECT ON department_attendance_stats TO authenticated;
GRANT SELECT ON classroom_attendance_stats TO authenticated;
...
GRANT SELECT ON department_attendance_stats TO anon;
GRANT SELECT ON classroom_attendance_stats TO anon;
...
SELECT * FROM classroom_attendance_stats;
