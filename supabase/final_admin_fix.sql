...
-- This script completely fixes all issues with the admin panel and database

-- 1. Disable RLS for all tables to avoid permission issues
...
-- 2. Grant all privileges to authenticated users
...
-- 5. Insert classrooms for all departments and year levels
-- Each department gets 1/1, 1/2, 1/3 for all year levels (ปวช.1-3, ปวส.1-2)
DO $$
DECLARE
    dept_id UUID;
    dept_name VARCHAR;
    ...
    FOR dept_id, dept_name IN SELECT id, name FROM departments LOOP
        FOR year_label, year_num IN SELECT * FROM jsonb_each_text(year_level_map) LOOP
            FOR room_num IN 1..3 LOOP
                ...
                (dept_name || ' ' || year_label || '/' || room_num, dept_id, year_num);
            END LOOP;
        END LOOP;
    END LOOP;
END $$;
...
VALUES (
    ...
    '0987654321',
    (SELECT id FROM classrooms WHERE name = 'เทคโนโลยีสารสนเทศฯ ปวช.1/1' LIMIT 1),
...
-- 13. Create view for dashboard stats
